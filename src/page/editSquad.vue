        ...
        <div class="ibox float-e-margins animated fadeInRight">
            ...
            <div class="ibox-content">
                <div class="form-horizontal">
                    ...
                    <!-- 是否保存模板 -->
                    <div class="form-grou col-sm-offset-2" v-if="targetID == 1">
                        ...
                        <input
                            ...
                            @blur="exist_template()">
                        <p class="tip" v-if="exist">already exist</p>
                        <div v-if="errors.first('vee-template')" class="tip">{{ errors.first('vee-template') }}</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="ibox-content">
                <div class="form-horizontal">
                   <div v-if="targetID == 1">
                    ...
                    <!-- Schedule -->
                    <div class="form-group">
                        <label class="col-sm-2 control-label"><i class="star">*</i>Schedule</label>
                        <div class="col-sm-5">
                            <el-date-picker
                                v-model="date"
                                type="datetimerange"
                        ...
                        <div class="col-sm-9">
                            <div>
                                ...
                                <div class="planDate">
                                    ...
                                    <span v-for="(item,index) in planHour" @click="getPlanHourSpan(item,index)" :id="item" v-if="addActiveClass(item)"></span>
...
export default {
    ...
    mounted:function(){
        ...
        .then(res=>{
            this.common_initialize(res); //公共
            //ndsp
            if(this.targetID == 0){
                this.ndsp_initialize(res);
            }
            //猎豹
            else if(this.targetID == 1){
                this.cheetah_initialize(res);
            }
          this.onDateChangeListener();
        })
    },
    methods:{
        onDateChangeListener: function () {
          ...
          // console.warn();
          // console.warn(this.date[1] - timestamp);
        ...
        //公共
        common_initialize:function(res){
            this.targetID = res.data.data.adSquadEntity.targetPlatform; //目标平台
            this.status = res.data.data.adSquadEntity.status; //状态
            this.list_target = res.data.data.target; //目标平台下拉框
            this.name = res.data.data.adSquadEntity.name; //名字
            this.campaignID = res.data.data.adSquadEntity.tbCampaignId; //campaignID
            this.load_campaign(); //加载campaign下拉框
            this.list_language = res.data.data.languages; //语言
            this.TotalBudget = res.data.data.adSquadEntity.budgetOverall; //总预算
            this.DaliyBudget = res.data.data.adSquadEntity.budgetDaily; //日预算
            this.date = [res.data.data.adSquadEntity.startTime*1000, res.data.data.adSquadEntity.endTime*1000];
            this.hourJson = JSON.parse(res.data.data.adSquadEntity.adScheduling); //投放时间
            this.list_OS = res.data.data.os; //OS
            if(res.data.data.adSquadEntity.devices.length>0)
            {
                this.OSID = res.data.data.adSquadEntity.devices[0].os;
                this.load_version(this.get_osID(this.OSID)); //加载版本号
                var array_version = res.data.data.adSquadEntity.devices[0].osVersion.split('-'); //分割版本号
                this.versionID1 = array_version[0];
                this.versionID2 = array_version[1];
            }
            //国家
            if(res.data.data.adSquadEntity.countryAlpha2Codes){
                this.countryID = res.data.data.adSquadEntity.countryAlpha2Codes.split(',');
                this.list_country = res.data.data.countrys; //加载国家
                this.load_carriers(); //加载运营商
                this.load_region(); //加载州
            }
            //州
            if(res.data.data.adSquadEntity.regions != null){
                var regions = [];
                for(var i=0;i<res.data.data.adSquadEntity.regions.length;i++){
                    regions.push(res.data.data.adSquadEntity.regions[i].regionCode)
                }
                this.regionID = regions;
                // this.load_region(); //加载州
            }
            //城市
            if(res.data.data.adSquadEntity.citys != null){
                var citys = [];
                for(var i=0;i<res.data.data.adSquadEntity.citys.length;i++){
                    citys.push(res.data.data.adSquadEntity.citys[i].cityName)
                }
                this.cityID = citys;
                this.load_city();
            }
            //运营商
            if(res.data.data.adSquadEntity.carriers != null){
                var carriers = [];
                if (res.data.data.adSquadEntity.citys!=null){
                  for(var i=0;i<res.data.data.adSquadEntity.citys.length;i++){
                    carriers.push(res.data.data.adSquadEntity.carriers[i].carrierName)
                  }
                }
                this.carriersID = carriers;
            }
            //CPM\CPC?
            if(res.data.data.adSquadEntity.priceModel){
                this.modelID = res.data.data.adSquadEntity.priceModel; //CPC、CPM？
                //this.list_model = res.data.data.priceModel; //CPC、CPM？
            }
            //时间投放插件
            for(var i=1;i<=7;i++){
                for(var j=0;j<24;j++){
        ...
        //ndsp，初始化添加
        ndsp_initialize:function(res){
            this.list_device = res.data.data.devices; //设备
            this.list_iab1 = res.data.data.iab1list; //IAB
            this.deviceID.push(res.data.data.adSquadEntity.devices[0].deviceType);
            this.bid = res.data.data.adSquadEntity.bidPrice; //bid
            this.TotalBudget = res.data.data.adSquadEntity.budgetOverall;//总预算
            this.DaliyBudget = res.data.data.adSquadEntity.budgetDaily;//日预算
            //bidPercent
            if(res.data.data.adSquadEntity.bidPercent){
                this.bidp = res.data.data.adSquadEntity.bidPercent;
            }
            //语言
            if(res.data.data.adSquadEntity.languages){
                this.languageID =  res.data.data.adSquadEntity.languages.split(',');
            }
            //IAB
            this.iabShow = res.data.data.adSquadEntity.tbIab1Values.length; //iab长度
            var j = 1;
            for(var i=0;i<this.iabShow;i++){
                this['iab1_' + j] = res.data.data.adSquadEntity.tbIab1Values[i].value;
                //str数组转int数组
                var dataIntArr = this.strToint(res.data.data.adSquadEntity.tbIab1Values[i].name.split(','));
                this['iab2_' + j] = dataIntArr;
                j++;
            }
            if(this.iabShow == 1){
                this.loadIAB2_1();
            }
            else if(this.iabShow == 2){
                this.loadIAB2_1();
                this.loadIAB2_2();
            }
            else if(this.iabShow == 3){
                this.loadIAB2_1();
                this.loadIAB2_2();
                this.loadIAB2_3();
            }
            else if(this.iabShow == 4){
                this.loadIAB2_1();
                this.loadIAB2_2();
                this.loadIAB2_3();
                this.loadIAB2_4();
            }
            else if(this.iabShow == 5){
                this.loadIAB2_1();
                this.loadIAB2_2();
                this.loadIAB2_3();
                this.loadIAB2_4();
            ...
            //包含还是屏蔽
            if(res.data.data.adSquadEntity.includeApps != null){
                this.APPIndex = 1;
                this.APPS = res.data.data.adSquadEntity.includeApps;
                this.change_loadApps();
            }
            else if(res.data.data.adSquadEntity.excludeApps != null){
                this.APPIndex = 2;
                this.APPS = res.data.data.adSquadEntity.excludeApps;
                this.change_loadApps();
            }
            else{
                this.APPIndex = "";
        ...
        //猎豹，初始化添加
        cheetah_initialize:function(res){
            ...
            this.list_template = res.data.data.template; //模板
            this.bid2 = res.data.data.adSquadEntity.bidPrice; //bid
            this.TotalBudget2 = res.data.data.adSquadEntity.budgetOverall;//总预算
            this.DaliyBudget2 = res.data.data.adSquadEntity.budgetDaily;//日预算
            //猎豹os只有安卓
            this.list_OS = [
                {
                    id:1,
            ...
            //objective
            if(res.data.data.objective){
                this.list_objective = res.data.data.objective; //下拉列表
                this.objectiveID = res.data.data.adSquadEntity.objective;
            ...
            //第三方账号
            if(res.data.data.adSquadEntity.tbTargetAccountId){
                this.accountID = res.data.data.adSquadEntity.tbTargetAccountId;
                this.load_account();
            }
            //语言
            if(res.data.data.adSquadEntity.languages){
                this.languageID2 =  res.data.data.adSquadEntity.languages;
            }
            //Ad Format
            if(res.data.data.adSquadEntity.appShowType){
                this.FormatID = res.data.data.adSquadEntity.appShowType;
            }
            //兴趣
            if(res.data.data.adSquadEntity.interest){
                this.checked_Interests = this.strToint(res.data.data.adSquadEntity.interest.split(','));
            }
            //机型
            if(res.data.data.adSquadEntity.deviceBrandModel){
                this.checked_deviceModels = this.strToint(res.data.data.adSquadEntity.deviceBrandModel.split(','));
            }
            this.app = res.data.data.adSquadEntity.pkgName; //app
            this.appTypeID = res.data.data.adSquadEntity.appType; //app类型
            this.landPageID = res.data.data.adSquadEntity.landingPage; //landPage
            this.NetworkID = res.data.data.adSquadEntity.netType; //网络类型
            this.genderID = res.data.data.adSquadEntity.gender; //性别
            this.ageID = res.data.data.adSquadEntity.age; //年龄
            this.deliveryID = res.data.data.adSquadEntity.deliveryType; //产品要求不需要必选，后端要求必选
            this.FormatID = res.data.data.adSquadEntity.appShowType; //Ad Format
            this.tracking = res.data.data.adSquadEntity.impUrl; //imp url
            this.url = res.data.data.adSquadEntity.clickUrl; //clickUrl url
            //语言
            if(res.data.data.adSquadEntity.languages){
                this.languageI2 =  res.data.data.adSquadEntity.languages.split(',');
            }
        },
        ...
        addActiveClass:function(item) {
            ...
            for (var dayKey in this.hourJson){
                for(var i=0;i<this.hourJson[dayKey].length;i++) {
                    selectArray.push(dayKey+'-'+this.hourJson[dayKey][i])
                }
            }
            var flag = false;
            for(var j=0;j<selectArray.length;j++) {
                if(item==selectArray[j]) {
                    flag = true;
                    break;
                }
            }
            if(!flag) {
                $('#'+item).addClass("active");  //样式
                var dayHour = item.split('-');
                $('#day-'+ dayHour[1]).addClass("active");
                for(var i=0;i<this.selectHour.length;i++) {
                    if(this.selectHour[i] == item) {
        ...
        load_region:function(){
            axios.get('/option/select2Region',{
                params:{
                    countryCodes:this.countryID.join(',')
                }
            })
            .then( res =>{
        ...
        load_city:function(){
            axios.get('/option/select2City',{
                params:{
                    regionCodes:this.regionID.join(',')
                }
            })
            .then( res =>{
            ...
            this.$validator.validateAll().then((result) => {
                if (result & this.exist=="") {
                    ...
                    if(this.APPIndex == ""){
                        ...
                        Include_apps = this.APPS;
                        Exclude_apps = [];
                    }
                    else if(this.APPIndex == 2){  //屏蔽APP
                        Include_apps = [];
                        Exclude_apps = this.APPS;
                    }
                    if (this.checkDate){
                      this.$message({
