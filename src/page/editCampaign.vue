    ...
    mounted:function(){
        ...
        .then(res=>{
            if( res.data.status == 1){
                this.name = res.data.data.name; //名字
                this.status = res.data.data.status; //状态
                this.BudgetOverall = res.data.data.budgetOverall; //总预算
                this.BudgetDaily = res.data.data.budgetDaily; //日预算
                this.mediabuy = res.data.data.tbMediaBuyId; //mediabuyID
                this.advertiser = res.data.data.tbAdvertiserId; //广告主ID
            }
        })
